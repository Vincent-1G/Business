...
async function cloudUpload(file, path) { const response = await fetch(`${CLOUD.url.replace(/\/$/, "")}/storage/v1/object/${CLOUD.bucket}/${path}`, { method: "POST", headers: cloudHeaders(), body: file }); if (!response.ok) throw new Error(`Cloud upload failed: ${response.status}`); return `${CLOUD.url.replace(/\/$/, "")}/storage/v1/object/public/${CLOUD.bucket}/${path}`; }
async function cloudAdd(item) { const path = `${crypto.randomUUID()}-${item.file.name.replace(/[^a-z0-9._-]/gi, "-")}`; const publicUrl = await cloudUpload(item.file, path); const remote = await cloudRequest("/rest/v1/memories", { method: "POST", headers: { Prefer: "return=representation" }, body: JSON.stringify({ title: item.title, type: item.type, favorite: false, added_at: item.addedAt, storage_path: path, public_url: publicUrl }) }); return { ...item, cloudId: remote[0].id, publicUrl, storagePath: path }; }
async function cloudItems() { if (!cloudEnabled) return []; const rows = await cloudRequest("/rest/v1/memories?select=*&order=added_at.desc"); return rows.map(row => ({ id: `cloud-${row.id}`, cloudId: row.id, title: row.title, type: row.type, favorite: row.favorite, addedAt: row.added_at, publicUrl: row.public_url, storagePath: row.storage_path })); }
...
async function cloudDelete(item) { if (!cloudEnabled || !item.cloudId) return; await cloudRequest(`/rest/v1/memories?id=eq.${encodeURIComponent(item.cloudId)}`, { method: "DELETE" }); if (item.storagePath) await cloudRequest("/storage/v1/object/remove", { method: "POST", body: JSON.stringify({ prefixes: [`${CLOUD.bucket}/${item.storagePath}`] }) }); }
...
function renderHeroCover() {
  const coverId = Number(localStorage.getItem("hero-cover-id"));
  const cover = state.items.find(item => item.id === coverId && (item.type === "photo" || item.type === "video"));
...
function renderCollageDetails(hero) {
  ...
  slots.forEach((slot, index) => {
    ...
    media.src = item.type === "video" ? fileUrl(item) : fileUrl(item);
    if (item.type === "video") {
      media.muted = true;
      media.loop = true;
      media.autoplay = true;
...
function createCard(item) {
  ...
  if (item.type === "photo") { const image = document.createElement("img"); image.src = fileUrl(item); image.alt = item.title; thumb.appendChild(image); } else if (item.type === "video" && item.thumbnail) { const image = document.createElement("img"); image.src = thumbUrl(item); image.alt = `${item.title} video still`; thumb.appendChild(image); } else { thumb.innerHTML = `<span class="visual-placeholder">${item.type === "audio" ? "♫" : "a moving<br>memory"}</span>`; }
...
async function manageCard(event, item) { const action = event.target.dataset.action; if (!action) return; if (action === "rename") { state.editingItem = item; state.pendingFiles = []; $("#dialog-heading").textContent = "Give it a new title."; $("#dialog-file").textContent = `Renaming “${item.title}”`; $("#memory-title").value = item.title; $("#title-dialog").hidden = false; $("#memory-title").focus(); } if (action === "favorite") { item.favorite = !item.favorite; await updateItem(item); await cloudUpdate(item); await refresh(); } if (action === "hero") { localStorage.setItem("hero-cover-id", item.id); renderHeroCover(); showToast("This photo is now your favorite chapter."); } if (action === "delete" && confirm("Delete this memory?")) { await removeItem(item.id); await cloudDelete(item); if (Number(localStorage.getItem("hero-cover-id")) === item.id) localStorage.removeItem("hero-cover-id"); revokeItemUrls(item.id); await refresh(); } }
