...
function fileUrl(item) { if (!state.urls.has(item.id)) state.urls.set(item.id, URL.createObjectURL(item.file)); return state.urls.get(item.id); }
function thumbUrl(item) { if (!state.thumbUrls.has(item.id)) state.thumbUrls.set(item.id, URL.createObjectURL(item.thumbnail)); return state.thumbUrls.get(item.id); }
...
async function addSelectedFiles(files) { for (const file of files) { const type = file.type.startsWith("video/") ? "video" : file.type.startsWith("audio/") ? "audio" : file.type.startsWith("image/") ? "photo" : null; if (!type) { showToast(`${file.name} is not a supported photo, video, or audio file.`); continue; } state.pendingFiles = [file]; $("#dialog-file").textContent = file.name; $("#memory-title").value = file.name.replace(/\.[^/.]+$/, ""); $("#title-dialog").hidden = false; $("#memory-title").focus(); await new Promise(resolve => { state.resolveTitle = resolve; }); }
  await refresh(); }
async function savePending() { const title = $("#memory-title").value.trim(); if (!title) return; if (state.editingItem) { state.editingItem.title = title; await updateItem(state.editingItem); state.editingItem = null; $("#title-dialog").hidden = true; $("#dialog-heading").textContent = "Give it a little title."; await refresh(); return; } const file = state.pendingFiles[0]; if (!file) return; const type = file.type.startsWith("video/") ? "video" : file.type.startsWith("audio/") ? "audio" : "photo"; const submit = $("#dialog-submit"); submit.disabled = true; submit.textContent = type === "video" ? "Preparing memory..." : "Saving memory..."; let thumbnail = null; if (type === "video") { try { thumbnail = await generateThumbnail(file); } catch { showToast("Preview unavailable; saving the original video instead."); } } try { await addItem({ file, thumbnail, title, type, favorite: false, addedAt: Date.now() }); } catch { showToast("This memory could not be saved in this browser."); } state.pendingFiles = []; $("#title-dialog").hidden = true; $("#dialog-heading").textContent = "Give it a little title."; submit.disabled = false; submit.textContent = "Save memory ↗"; state.resolveTitle?.(); state.resolveTitle = null; }
async function refresh() { state.items = (await getAll()).sort((a, b) => b.addedAt - a.addedAt); render(); }
